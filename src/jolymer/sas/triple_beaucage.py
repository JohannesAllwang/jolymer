...
def triple_beaucage(q, Rg1, Rg2, Rg3, G1, G2, G3, d1, d2, d3):
    r"""
    ...
    Rcut1 = Rg2
    Rcut2 = Rg3
    B1 = G1 * d1 * Rg1**(-d1) * (6 * d1**2 * float(2+d1)**-1 * float(2 + 2*d1)**-1)**(d1/2)
    B2 = G2 * d2 * Rg2**(-d2) * (6 * d2**2 * float(2+d2)**-1 * float(2 + 2*d2)**-1)**(d2/2)
    B3 = G3 * d3 * Rg3**(-d3) * (6 * d3**2 * float(2+d3)**-1 * float(2 + 2*d3)**-1)**(d3/2)
    I1 = G1 * np.exp(-q**2 * Rg1**2 /3 ) + B1 * np.exp(-q**2 * Rcut1**2 / 3) * special.erf(q * Rg1 / np.sqrt(6))**(3*d1) * q**(-d1)
...
class TripleBeaucage(sasmodel.SAXS_Model):
    
    def __init__(self):
        self.name = 'beaucage'
        self.longname = 'Beaucage'
        self.parameters = ['Rg1', 'Rg2',  'Rg3', 'G1', 'G2', 'G3', 'd1', 'd2', 'd3']
        self.pdict = {'xi' : ['$\\xi$', 'nm']}
        self.fitfunc = triple_beaucage
    ...
    def get_text(self, fit_dict):
        try: 
            ...
            """.format(fit_dict['Rg2'], fit_dict['std_Rg2'],
                   ...
                   fit_dict['chi2'],
                   fit_dict['bg'],
                   fit_dict['Rg1'], fit_dict['std_Rg1'],
                   fit_dict['G1'], fit_dict['std_G1'])
...
tbeaucage = TripleBeaucage().plusmodel(background)
